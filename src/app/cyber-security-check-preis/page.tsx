    ...
    <main className="bg-[#0A0A0B] text-white min-h-screen">
      ...
      <section className="premium-hero py-20 lg:py-28 border-b border-white/10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
            ...
            <span className="text-[#FF3B30]">Aktuelle Preisliste</span>
          </h1>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-white/15">
                  <th className="text-left py-3 px-4 text-white/60 text-sm font-semibold">Service</th>
                  <th className="text-left py-3 px-4 text-white/60 text-sm font-semibold">Preis von</th>
                  <th className="text-left py-3 px-4 text-white/60 text-sm font-semibold">Preis bis</th>
                  <th className="text-left py-3 px-4 text-white/60 text-sm font-semibold">Dauer</th>
                </tr>
              </thead>
              <tbody>
                {PRICE_LIST.map((row, i) => (
                  <tr key={row.service} className={`border-b border-white/10 ${i % 2 === 0 ? "" : "bg-white/[0.02]"}`}>
                    ...
                    <td className="py-4 px-4 text-white/70 text-sm">{row.priceTo}</td>
                    <td className="py-4 px-4 text-white/60 text-sm">{row.time}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <p className="text-white/50 text-xs mt-3">* Alle Preise netto zzgl. MwSt. · Richtwerte – individuelles Angebot nach Scoping</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {["Kostenloses Scoping-Gespräch", "Manuelles Assessment durch zertifizierte Experten", "Priorisierter Sicherheitsbericht (Deutsch)", "Executive Summary für Management", "Technischer Bericht mit PoC", "1 kostenloser Retest", "Remediation Guidance", "Persönliches Abschlussgespräch"].map((item) => (
              <div key={item} className="flex items-start gap-3 bg-[#0A0A0B] border border-white/10 rounded-2xl p-4">
      ...
      <section className="py-14 border-t border-white/10 text-center">
        <div className="max-w-2xl mx-auto px-4">
          <Shield className="w-10 h-10 text-[#FF3B30] mx-auto mb-4" />
          <h2 className="text-2xl font-bold mb-4">Preis anfragen</h2>
          <p className="text-white/60 mb-6">Festpreisangebot in 24 Stunden · keine versteckten Kosten</p>
          ...
          <p className="text-white/50 text-sm mt-5">
            ...
            <Link href="/cyber-security-check-kosten" className="text-[#FF3B30] hover:text-[#FF6B61]">Kosten</Link>
          </p>
        </div>
      </section>
    </main>
