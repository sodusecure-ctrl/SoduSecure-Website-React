...
export default function SMESecurityPackagesLanding() {
  const router = useRouter();
  const t = useTranslations('smePackages');
  const includedItems = [
    { text: t('whatsIncluded.items.0'), featured: false },
    { text: t('whatsIncluded.items.1'), featured: true },
    { text: t('whatsIncluded.items.2'), featured: false },
    { text: t('whatsIncluded.items.3'), featured: false },
    { text: t('whatsIncluded.items.4'), featured: false },
    { text: t('whatsIncluded.items.5'), featured: false },
    { text: t('whatsIncluded.items.6'), featured: false },
    { text: t('whatsIncluded.items.7'), featured: false }
  ];

  const scopeItems = [
    { icon: DollarSign, title: t('scopeCoverage.items.webApplications.title'), desc: t('scopeCoverage.items.webApplications.description') },
    { icon: Building2, title: t('scopeCoverage.items.networkInfrastructure.title'), desc: t('scopeCoverage.items.networkInfrastructure.description') },
    { icon: CheckCircle, title: t('scopeCoverage.items.basicCompliance.title'), desc: t('scopeCoverage.items.basicCompliance.description') },
    { icon: Target, title: t('scopeCoverage.items.vulnerabilityScanning.title'), desc: t('scopeCoverage.items.vulnerabilityScanning.description') },
    { icon: Shield, title: t('scopeCoverage.items.configurationReview.title'), desc: t('scopeCoverage.items.configurationReview.description') },
    { icon: Shield, title: t('scopeCoverage.items.accessControls.title'), desc: t('scopeCoverage.items.accessControls.description') },
    { icon: Briefcase, title: t('scopeCoverage.items.dataProtection.title'), desc: t('scopeCoverage.items.dataProtection.description') },
    { icon: FileText, title: t('scopeCoverage.items.reporting.title'), desc: t('scopeCoverage.items.reporting.description') }
  ];
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {includedItems.map((item, index) => (
              <div
                key={index}
                className={`flex items-start gap-3 p-3 sm:p-4 rounded-lg transition-all ${item.featured
                  ? 'hover:bg-gray-50'
                  : 'hover:bg-gray-50'
                  }`}
              >
                <div className={`w-5 h-5 sm:w-6 sm:h-6 rounded flex items-center justify-center flex-shrink-0 mt-0.5 sm:mt-1 ${item.featured ? 'bg-black' : 'bg-black'}`}>
                  <svg className="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <span className={`text-xs sm:text-sm leading-relaxed ${item.featured ? 'font-medium text-blue-900' : ''}`}>
                  {item.text}
                </span>
              </div>
            ))}
