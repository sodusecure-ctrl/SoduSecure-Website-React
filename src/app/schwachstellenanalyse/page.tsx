...
const ANALYSEBEREICHE = [
  ...
  { icon: AlertTriangle, title: "Netzwerk Schwachstellenanalyse", desc: "Port-Scanning, Service-Fingerprinting, Banner-Grabbing, bekannte Exploits auf alle exponierten Dienste.", color: "text-[#FF3B30]", bg: "bg-[#FF3B30]/10 border-[#FF3B30]/20" },
    ...
    <main className="bg-[#0A0A0B] text-white min-h-screen">
      ...
      <section className="py-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-white/60 text-sm">Was ist der Unterschied – und wann brauchen Sie was?</p>
        </div>
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
          ...
          {VERGLEICH_ROWS.map((row, i) => (
            <div key={i} className={`grid grid-cols-3 px-6 py-4 text-sm border-b border-white/5 last:border-0 ${i % 2 === 0 ? '' : 'bg-white/[0.01]'}`}>
              <span className="text-white/60 text-xs pr-2">{row.kriterium}</span>
              <span className="text-xs">
                {row.analyse === true ? <CheckCircle className="w-4 h-4 text-green-400" /> : row.analyse === false ? <span className="text-white/70">✕</span> : <span className="text-orange-400">{row.analyse}</span>}
              </span>
              <span className="text-xs">
                {row.pentest === true ? <CheckCircle className="w-4 h-4 text-green-400" /> : row.pentest === false ? <span className="text-white/70">✕</span> : <span className="text-green-400">{row.pentest}</span>}
              </span>
            </div>
          ))}
        </div>
