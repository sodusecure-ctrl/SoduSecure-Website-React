...
async function sendEmails(session: Stripe.Checkout.Session) {
  ...
  const interval: BillingInterval = isBillingInterval(meta.billingInterval)
    ? meta.billingInterval
    : 'month';
  const pricing = plan ? getPlanPricing(plan, interval) : null;
  const intervalLabel = interval === 'year' ? 'jährlich' : 'monatlich';

  const repoList = meta.repoUrls
          ...
          <body style="margin:0;padding:0;font-family:'Segoe UI',Arial,sans-serif;background:#0A0A0B;">
            <div style="max-width:680px;margin:30px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.18);">
              <div style="background:linear-gradient(135deg,#0A0A0B 0%,#10B981 100%);padding:32px;">
                <div style="color:rgba(255,255,255,.78);font-size:11px;letter-spacing:.22em;text-transform:uppercase;">Zahlung bestätigt · ${e.interval}</div>
              ...
              <div style="padding:30px;">
                <table style="width:100%;border-collapse:collapse;">
                  ...
                  <tr><td style="padding:9px 0;color:#6B7280;font-size:13px;vertical-align:top;">Repositories</td><td style="padding:9px 0;color:#0A0A0B;font-size:14px;"><ul style="margin:0;padding-left:18px;">${reposHtml || '<li>(keine übermittelt)</li>'}</ul></td></tr>
          ...
          <body style="margin:0;padding:0;font-family:'Segoe UI',Arial,sans-serif;background:#F4F4F5;">
            <div style="max-width:640px;margin:30px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,.10);">
              <div style="background:linear-gradient(135deg,#0A0A0B 0%,#FF3B30 100%);padding:36px;">
                ...
                <p style="color:rgba(255,255,255,.85);margin:8px 0 0;font-size:15px;">Deine Zahlung für <strong>${e.label}</strong> (${e.interval}) ist eingegangen.</p>
              </div>
              <div style="padding:32px;">
                ...
                <div style="margin:22px 0;padding:16px 18px;background:#FAFAFA;border:1px solid #E4E4E7;border-radius:10px;">
                  ...
                  <div style="margin-top:8px;color:#0A0A0B;font-size:14px;line-height:1.7;">
                    <div><strong>Plan:</strong> ${e.label} · ${e.amount} (${e.interval})</div>
                    ${reposPlainHtml ? `<div style="margin-top:6px;"><strong>Repositories:</strong></div>${reposPlainHtml}` : ''}
                  </div>
                </div>
              ...
              <div style="background:#0A0A0B;padding:20px 32px;color:rgba(255,255,255,.55);font-size:12px;">Sodu Secure · sodusecure.com · ${interval === 'year' ? 'jährlich, kündbar zum Laufzeitende' : 'monatlich kündbar'}</div>
            </div>
          </body></html>
        `,
      });
