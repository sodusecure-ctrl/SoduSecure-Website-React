    ...
    <main className="bg-[#0A0A0B] text-white min-h-screen">
      ...
      <section className="premium-hero py-20 lg:py-28 border-b border-white/10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 bg-[#FF3B30]/10 border border-[#FF3B30]/20 rounded-full px-4 py-1.5 mb-6">
            ...
            <span className="text-[#FF3B30] text-sm font-medium">Transparente Preise 2025</span>
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
            Cyber Security Check Kosten –<br />
            <span className="text-[#FF3B30]">Was kostet ein Check?</span>
          </h1>
      ...
      <section className="py-16 lg:py-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {PRICE_TABLE.map((pkg) => (
              <div key={pkg.name} className={`rounded-xl p-6 border ${pkg.featured ? "bg-[#1a0a0a]/20 border-[#FF3B30]/40" : "bg-[#0A0A0B] border-white/10"}`}>
                {pkg.featured && <div className="text-yellow-400 text-sm font-semibold mb-3">⭐ Empfohlen für KMU</div>}
                <h3 className="font-bold text-lg mb-1">{pkg.name}</h3>
                <div className="text-2xl font-bold text-[#FF3B30] mb-1">{pkg.price}</div>
                <p className="text-white/50 text-sm mb-1">{pkg.duration} · {pkg.scope}</p>
                ...
                <a href={PHONE_HREF} className="mt-6 w-full inline-flex items-center justify-center premium-cta text-white px-4 py-3 rounded-2xl text-sm font-medium transition-colors">
                  Angebot anfragen
                </a>
              </div>
            ))}
      ...
      <section className="py-16 bg-[#0A0A0B]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {FACTORS.map((f) => (
              <div key={f.title} className="bg-[#0A0A0B] border border-white/10 rounded-xl p-5">
      ...
      <section className="py-14 border-t border-white/10 text-center">
        <div className="max-w-2xl mx-auto px-4">
          ...
          <p className="text-white/60 mb-6">Kostenlose Erstberatung · Festpreisangebot in 24h</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href={PHONE_HREF} className="inline-flex items-center justify-center gap-2 premium-cta text-white px-8 py-4 rounded-2xl font-semibold transition-colors">
