...
const PentestPreisrechner = () => {
  ...
  // Mails mehr aus – erst wieder das abgeschickte Formular selbst bzw. die
  // Abbruch-Mail beim Verlassen der Seite.
  const hasTrackedStart = useRef(false);

  // ── Funnel-Protokoll: jede Interaktion wird chronologisch festgehalten ──
  const eventLog = useRef<string[]>([]);
  const loggedFields = useRef<Set<string>>(new Set());
  const maxStepRef = useRef(1);
  const submittedRef = useRef(false);
  const lastBeaconKey = useRef('');

  const logEvent = (msg: string) => {
    eventLog.current.push(`${new Date().toLocaleTimeString('de-DE')} – ${msg}`);
  };

  // Aktueller Formularzustand für die Abbruch-Mail (Event-Listener sehen sonst
  // nur veraltete State-Werte).
  const liveRef = useRef({ currentStep, selectedCategory, selectedScope, selectedAddOns, selectedReport, formData });
  liveRef.current = { currentStep, selectedCategory, selectedScope, selectedAddOns, selectedReport, formData };

  // Abbruch-Erkennung: Verlässt der Besucher die Seite (Tab zu, Navigation,
  // Tab-Wechsel), ohne abgesendet zu haben, geht ein letzter Stand per
  // sendBeacon raus – inkl. aller bereits eingegebenen Daten und dem Verlauf.
  useEffect(() => {
    const sendAbandon = () => {
      if (submittedRef.current) return;
      const s = liveRef.current;
      const hasInput = Object.values(s.formData).some((v) => typeof v === 'string' && v.trim() !== '');
      if (!s.selectedCategory && !hasInput) return; // nichts Nennenswertes passiert
      const payload = JSON.stringify({
        action: 'form-abandoned',
        currentStep: s.currentStep,
        maxStep: maxStepRef.current,
        selectedCategory: s.selectedCategory,
        selectedScope: s.selectedScope,
      ...
      if (lastBeaconKey.current === payload) return; // identischen Stand nicht doppelt mailen
      lastBeaconKey.current = payload;
      const blob = new Blob([payload], { type: 'application/json' });
      if (!(navigator.sendBeacon && navigator.sendBeacon('/api/pentest/track', blob))) {
        void fetch('/api/pentest/track', {
          method: 'POST',
  ...
  const handleNext = () => {
    ...
    maxStepRef.current = Math.max(maxStepRef.current, newStep);
    logEvent(`Weiter zu Schritt ${newStep}`);
  ...
  const handleCategoryNext = () => {
    ...
    if (selectedCategory === 'other') {
      maxStepRef.current = 4;
      logEvent('Sonderanfrage: direkt zu Schritt 4');
      setCurrentStep(4);
    } else {
      handleNext();
      return;
  ...
  const handleBack = () => {
    ...
    logEvent(`Zurück zu Schritt ${newStep}`);
    setCurrentStep(newStep);
    scrollToTop();
  };
  ...
  const handleCategorySelect = (id: string) => {
    ...
    logEvent(`Schritt 1: Kategorie "${title}" gewählt`);
  ...
  const toggleAddOn = (id: string) => {
    logEvent(`Add-On "${id}" ${selectedAddOns[id] ? 'entfernt' : 'aktiviert'}`);
    setSelectedAddOns(prev => ({ ...prev, [id]: !prev[id] }));
  };

  const fieldLogLabels: Record<string, string> = {
    fullName: 'Name',
    companyName: 'Firma',
    email: 'E-Mail',
    phone: 'Telefon',
  ...
  const handleInputChange = (field: keyof FormData, value: string | boolean) => {
    if (field === 'agreed') {
      if (value === true) logEvent('Datenschutz-Checkbox angehakt');
    } else if (typeof value === 'string' && value.trim() !== '' && !loggedFields.current.has(field)) {
      loggedFields.current.add(field);
      logEvent(`Feld "${fieldLogLabels[field] || field}" ausgefüllt`);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      submittedRef.current = true; // keine Abbruch-Mail mehr nach erfolgreichem Absenden
      trackConversion();
      if (priceRange) {
                  ...
                  <div className={`grid gap-3 ${getScopeOptions(selectedCategory).length === 1 ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-3'}`}>
                    {getScopeOptions(selectedCategory).map((scope) => {
                      ...
                      return (
                        ...
                        >
                          <button
                            onClick={() => { logEvent(`Schritt 2: Prüfumfang "${scope.title}" gewählt`); setSelectedScope(scope.id); }}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                    {reportOptions.map((opt) => {
                      ...
                      return (
                        <button
                          key={opt.id}
                          onClick={() => { logEvent(`Schritt 3: Prüfabschluss "${opt.title}" gewählt`); setSelectedReport(opt.id); }}
