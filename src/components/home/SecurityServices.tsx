  ...
  const services = [
    {
      ...
      tagline: t('infrastructure.tagline'),
      color: 'from-red-500 to-red-600'
    },
    {
      icon: Globe,
      title: t('webApplication.title'),
      route: "/services/web-application-testing",
      description: t('webApplication.description'),
      tagline: t('webApplication.tagline'),
    ...
    },
    {
      ...
      tagline: t('cloudDevops.tagline'),
      color: 'from-orange-600 to-red-700'
    },
    {
      icon: Users,
      title: t('networkSecurity.title'),
      route: "/services/network-audit",
      description: t('networkSecurity.description'),
      tagline: t('networkSecurity.tagline'),
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={service.title}
                className="group relative bg-[#16141A] rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg hover:shadow-xl lg:hover:shadow-2xl transition-all duration-500 hover:-translate-y-1 lg:hover:-translate-y-2 animate-fade-in border border-white/8 overflow-hidden"
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  {/* Title */}
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2 sm:mb-3 group-hover:text-[#FF3B30] transition-colors duration-300">
                    {service.title}
                  </h3>
                  ...
                  {/* CTA Link */}
                  <button onClick={() => router.push(service.route)} className="group/btn cursor-pointer flex items-center gap-1 sm:gap-2 text-[#FF3B30] font-semibold text-sm sm:text-base hover:gap-2 sm:hover:gap-3 transition-all duration-300">
                    {t('learnMore')}
        ...
        <div className="mt-12 sm:mt-16 lg:mt-20 text-center animate-fade-in" style={{ animationDelay: '0.6s' }}>
          <div className="inline-flex flex-col items-center gap-3 sm:gap-4 bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl sm:rounded-2xl p-6 sm:p-8 shadow-xl sm:shadow-2xl w-full sm:w-auto">
            <p className="text-white text-base sm:text-lg font-medium">
              {t('customAssessment.title')}
            </p>
            <button onClick={() => router.push("/contact")} className="bg-red-600 cursor-pointer hover:premium-cta text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg font-semibold transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-red-500/50 text-sm sm:text-base w-full sm:w-auto">
              {t('customAssessment.button')}
            </button>
          </div>
        </div>
      </div>
